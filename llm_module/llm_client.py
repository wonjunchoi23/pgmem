...
- Together AI
- vLLM
- OpenAI (also supports OpenAI-compatible endpoints via base_url)

Features:
- System prompt support (chat format)
- JSON mode (OpenAI-compatible response_format)
- Structured output (vLLM StructuredOutputsParams / GuidedDecodingParams)
- Optional JSON validation with retry logic
- OpenAI reasoning model auto-detection (o1*, o3*, o4*, gpt-5*)
- Batch generation for vLLM and OpenAI (concurrent)
"""
...
# Structured output API: vLLM >=0.10 renamed GuidedDecodingParams to
# StructuredOutputsParams and the SamplingParams field 'guided_decoding' to
# 'structured_outputs'. Try the new name first, fall back to the legacy name.
_StructuredOutputsCls = None
_STRUCTURED_OUTPUT_KEY = None
if VLLM_AVAILABLE:
    try:
        from vllm.sampling_params import StructuredOutputsParams as _StructuredOutputsCls
        _STRUCTURED_OUTPUT_KEY = "structured_outputs"
    except ImportError:
        try:
            from vllm.sampling_params import GuidedDecodingParams as _StructuredOutputsCls
            _STRUCTURED_OUTPUT_KEY = "guided_decoding"
        except ImportError:
            _StructuredOutputsCls = None
            _STRUCTURED_OUTPUT_KEY = None
            logging.warning(
                "vLLM is installed but neither StructuredOutputsParams nor "
                "GuidedDecodingParams is available; JSON schema enforcement "
                "will be disabled and only JSON-retry fallback will apply."
            )

# Backwards-compatible aliases so external imports like
# `from llm_client import GuidedDecodingParams` keep working.
GuidedDecodingParams = _StructuredOutputsCls
StructuredOutputsParams = _StructuredOutputsCls


def _make_structured_kwargs(json_schema: Optional[dict]) -> Dict[str, Any]:
    """Build the SamplingParams kwarg for structured JSON output.

    Returns {} when vLLM has no structured-output support or schema is None,
    so the caller can safely `sampling_kwargs.update(...)`.
    """
    if not json_schema or _StructuredOutputsCls is None or _STRUCTURED_OUTPUT_KEY is None:
        return {}
    return {_STRUCTURED_OUTPUT_KEY: _StructuredOutputsCls(json=json_schema)}


# Legacy alias for callers that imported the old name.
_make_guided_sampling_kwargs = _make_structured_kwargs
...
class OpenAIClient(BaseLLMClient):
    ...
    Reasoning model handling:
      - Auto-detected for o1*, o3*, o4*, gpt-5* model names: max_tokens is
        rewritten to max_completion_tokens (floored at 4096) and
        temperature/top_p are dropped.
      - `reasoning_mode=True` forces the same rewrite regardless of the model
        name and uses REASONING_MAX_COMPLETION_TOKENS as the explicit cap.
        Useful for OpenAI-compatible endpoints whose reasoning models do not
        follow the public OpenAI naming convention.
    """

    REASONING_MAX_COMPLETION_TOKENS = 4000
        ...
        **kwargs
        ...
        if self.reasoning_mode:
            api_kwargs = {
                ...
                "messages": msgs,
                "max_completion_tokens": self.REASONING_MAX_COMPLETION_TOKENS,
                "temperature": 1,
                **{k: v for k, v in kwargs.items()
                   if k not in ("max_tokens", "temperature", "return_usage", "json_retry")},
            }
        else:
            api_kwargs = {"model": self.model_name, "messages": msgs,
