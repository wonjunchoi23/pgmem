"""
Configuration for Theanine Batch Experiment on PersonaMem
...
# --max-model-len CLI arg (see run_experiment.py). 85% utilization absorbs
# tokenizer mismatch (tiktoken vs Llama SentencePiece) and template overhead.
FINALIZE_INPUT_CONTEXT_LIMIT = DEFAULT_VLLM_CONFIG["max_model_len"]
FINALIZE_CONTEXT_UTILIZATION = 0.85

# Number of sub-blocks to split each block into for finalize granularity.
# 1M has longer blocks so we finalize twice per block to reduce memory lag.
...
# 1 PersonaMem block = 1 virtual day (matches amem / memorybank).
CONV_IDS_PER_DAY       = 1
MINUTES_PER_TURN       = 10
