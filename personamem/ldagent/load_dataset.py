...
questions_[SIZE].csv  — one row per QA question:
    ...
    distance_to_ref_proportion_in_context, context_length_in_letters
...
shared_contexts_[SIZE].jsonl  — one JSON object per line:
    {"<shared_context_id>": [
        ...
        {"role": "assistant", "content": "Assistant: That sounds ..."},
        ...
    ]}

Block structure:
    A "block" is the sequence of user/assistant messages that follows a
    system message. block_idx increments at each system message.
    local_msg_idx counts only user/assistant messages within each block.

Usage:
------
    from load_dataset import load_personamem_dataset, PersonaMemContext

    contexts = load_personamem_dataset(questions_path, contexts_path)

    for context in contexts:
        # context is sorted by QA count descending at the dataset level
        for msg in context.messages:   # user/assistant only, system excluded
            print(msg.role, msg.content[:60])
        for qa in context.qa_pairs:    # sorted by end_index ascending
            print(qa.question, qa.correct_answer)
"""
...
class PersonaMemMessage:
    ...
    def to_memory_content(self) -> str:
        """Return content as-is for A-MEM storage."""
        return self.content
...
def load_personamem_dataset(
    ...
    # Build offset index for JSONL random access
    jsonl_index = _build_jsonl_index(contexts_path)

    # ---- Read questions CSV and group by shared_context_id ----
    ...
    # ---- Build PersonaMemContext objects ----
...
# =============================================================================
# LD-AGENT EXTRAS — virtual time + message pairing
# =============================================================================

def compute_virtual_seconds(
    block_idx: int,
    local_msg_idx: int,
    conv_ids_per_day: int = 1,
    minutes_per_turn: int = 10,
) -> float:
    """Virtual time (seconds since context start) for one PersonaMem message.

    Mirrors ImplexConv LD-Agent's compute_virtual_seconds with
    block_idx in the conv_id slot and local_msg_idx in the turn_id slot.
    """
    minutes_per_day  = 1440.0
    minutes_per_half = minutes_per_day / conv_ids_per_day
    day  = block_idx // conv_ids_per_day
    half = block_idx %  conv_ids_per_day
    virtual_minutes = day * minutes_per_day + half * minutes_per_half + local_msg_idx * minutes_per_turn
    return virtual_minutes * 60.0


def convert_seconds_to_full_time(seconds: float) -> str:
    """'XX years XX months XX days XX hours XX minutes' (only non-zero units)."""
    units = [
        ("years",   31536000),
        ("months",   2592000),
        ("days",       86400),
...
def get_message_pairs(
    messages: List[PersonaMemMessage],
) -> List[tuple]:
    """Pair PersonaMem messages into (user, assistant) tuples.

    Pairing crosses block boundaries (Q8=A): user followed by an assistant
    forms a pair regardless of block_idx. unpaired user → (user, None).
    Lone assistant (rare edge case) is dropped.
    """
    pairs = []
    i = 0
    n = len(messages)
    while i < n:
        ...
        if m.role == "user":
            if i + 1 < n and messages[i + 1].role == "assistant":
                pairs.append((m, messages[i + 1]))
                i += 2
            else:
                pairs.append((m, None))
                i += 1
        else:
            # lone assistant without a preceding user — skip
            i += 1
    return pairs


def strip_speaker_prefix(content: str) -> str:
    """Strip 'User: ' / 'Assistant: ' prefix from a PersonaMem message content."""
...
if __name__ == "__main__":
    ...
    print(f"  options : {len(q.all_options)} options, first = {q.all_options[0][:60]}")
