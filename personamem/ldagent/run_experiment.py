"""
LD-Agent Experiment Runner — PersonaMem (Batched, QA-Only)

Runs multiple PersonaMem shared contexts in parallel within one GPU by
batching LD-Agent's internal LLM calls (boundary summarisation, persona
extraction, QA). Memory states remain isolated per context.

Internal field names (`session_id`, `conv_id`, `turn_id`) inside
event_memory / personas / ldagent_module are retained from the ImplexConv
variant. The runner injects PersonaMem identifiers into those slots:
  context_index → session_id    block_idx → conv_id    local_msg_idx → turn_id
Result metadata is remapped via `_remap_retrieved_memories`.

Phase 1 iterates over (user, assistant) pairs (Q1=A) constructed by
`get_message_pairs()` (block-boundary-crossing pairing, Q8=A). Each message's
content is stripped of its "User: "/"Assistant: " prefix before being passed
to LD-Agent (Q2). Unpaired user messages update only the user persona and
add only the user turn to STM (Q5).

Phase 2 does NOT call `flush_to_ltm` (Q7=B). STM is retained as the QA
<CONTEXT> exactly as left by Phase 1 (= the last block's messages, since
FINALIZE_EVERY_N_CONVS = CONV_IDS_PER_DAY = 1 clears STM at every block
boundary).
"""
...
from load_dataset import (
    ...
    PersonaMemMessage,
    PersonaMemQAPair,
    compute_virtual_seconds,
    get_message_pairs,
    strip_speaker_prefix,
)
from ldagent_module import LDAgentModule, LLMCallLogger
from generator import format_memories_for_prompt, QA_SCHEMA_MULTICHOICE
...
def print_completion_banner(results_file: Path, total_contexts: int):
    banner = "#" * 60
    print(f"\n{banner}\n# Experiment completed — {total_contexts} contexts processed")
...
def _build_retrieval_log_entry(
    ...
    for mem in relevant_memories:
        retrieved_items.append({
            ...
            "source_turn": {
                "context_index":   mem.get("session_id", 0),
                "block_idx":       mem.get("conv_id",    0),
...
def _remap_retrieved_memories(memories: List[Dict]) -> List[Dict]:
    """Rename event_memory keys (session_id/conv_id) to PersonaMem keys."""
    return [
        {
            "context_index":   mem.get("session_id", -1),
            "block_idx":       mem.get("conv_id",    -1),
            "virtual_seconds": mem.get("virtual_seconds", 0.0),
            "score":           mem.get("score", 0.0),
        }
        for mem in memories
...
class BatchedLDAgentRunner:
    def __init__(
        ...
        self.retrieval_log_dir    = cfg.get_retrieval_log_dir(
            model_path, benchmark_size, start_session, end_session, config_name
        )
    ...
    # ------------------------------------------------------------------
    # Batch generation with sequential JSON-retry fallback
    # ------------------------------------------------------------------
    ...
    def _batch_generate_with_retry(
        ...
        if hasattr(inner, "llm") and hasattr(inner, "_build_prompt") and hasattr(inner, "_get_json_schema"):
            ...
            sampling_kwargs.update(_make_structured_kwargs(json_schema))
    ...
    def _run_phase1_batched(
        ...
        """Pair-aligned iteration across contexts. Per-pair flow:
          1. Boundary summarise (per-context check) — batched
          2. Append user query to STM (in-memory only) — per-context
          3. Batch user persona extraction
          4. Batch agent persona extraction (skip pairs with no assistant)
          5. Append GT agent response to STM — per-context
        """
        phase1_stats = [
            {
        ...
        pairs_list = [get_message_pairs(c.messages) for c in contexts]
        max_pairs  = max((len(p) for p in pairs_list), default=0)

        for pair_idx in tqdm(range(max_pairs), desc="Phase1 pairs"):
            active = []
            for i, (context, agent, pairs) in enumerate(zip(contexts, agents, pairs_list)):
                if pair_idx >= len(pairs):
                    continue
                user_msg, assistant_msg = pairs[pair_idx]
                if user_msg is None:
                    # lone assistant — should not happen in practice (skipped in get_message_pairs)
                    continue
                bsize = block_sizes_list[i].get(user_msg.block_idx, 1)
                sub = min(user_msg.local_msg_idx * chunk_factor // bsize, chunk_factor - 1)
                active.append({
                    "ctx_idx":         i,
                    "context":         context,
                    "agent":           agent,
                    ...
                    "user_utterance":  strip_speaker_prefix(user_msg.content),
                    "agent_response":  (
                        strip_speaker_prefix(assistant_msg.content)
                        if assistant_msg is not None else ""
                    ),
                    "virtual_seconds": compute_virtual_seconds(
                        user_msg.block_idx,
                        user_msg.local_msg_idx,
                        cfg.CONV_IDS_PER_DAY,
                        cfg.MINUTES_PER_TURN,
                    ),
                    "virtual_conv_id": user_msg.block_idx * chunk_factor + sub,
                })

            if not active:
                continue

            # --- Boundary summarisation ---
            boundary_batch = []
            for item in active:
                ...
                summary_job = memory_bank.prepare_boundary_summary(
                    ...
                    current_conv_id=item["virtual_conv_id"],
                    current_session_id=item["context"].context_index,
                )
            ...
            if boundary_batch:
                ...
                for entry, result, usage in zip(boundary_batch, results, usages):
                    ...
                    agent.memory_bank.apply_boundary_summary_result(
                        entry["summary_job"], summary_text
                    )
                    _add_tokens(
                        phase1_stats[entry["ctx_idx"]]["call_4_summarization"],
                        usage["prompt_tokens"], usage["completion_tokens"],
                    )
            ...
            for item in active:
                ...
                # Append user turn to STM (no LLM call)
                memory_bank.append_user_query(
                    query=item["user_utterance"],
                    current_virtual_seconds=item["virtual_seconds"],
                    current_conv_id=item["virtual_conv_id"],
                    current_session_id=item["context"].context_index,
                )

                sys_prompt, user_prompt = personas.build_user_trait_prompt(
                    item["user_utterance"]
                )
                turn_job = {
                    "ctx_idx":        item["ctx_idx"],
            ...
            if user_persona_batch:
                ...
                for entry, result, usage in zip(user_persona_batch, results, usages):
                    ...
                    _add_tokens(
                        phase1_stats[turn_job["ctx_idx"]]["call_2_user_persona"],
                        usage["prompt_tokens"], usage["completion_tokens"],
                    )

            # --- Agent persona extraction (skip pairs without assistant) ---
            agent_persona_batch = []
            for turn_job in turn_jobs:
                if turn_job["assistant_msg"] is None:
                    continue  # Q5=A: unpaired user — no agent persona update
                agent = turn_job["agent"]
                personas = agent.personas
                sys_prompt, user_prompt = personas.build_agent_trait_prompt(
                    turn_job["agent_response"]
                )
            ...
            if agent_persona_batch:
                ...
                for entry, result, usage in zip(agent_persona_batch, results, usages):
                    ...
                    _add_tokens(
                        phase1_stats[turn_job["ctx_idx"]]["call_3_agent_persona"],
                        usage["prompt_tokens"], usage["completion_tokens"],
                    )
                    agent.memory_bank.add_agent_response(
                        response=turn_job["agent_response"],
                        current_virtual_seconds=turn_job["virtual_seconds"],
                        current_session_id=turn_job["context"].context_index,
                    )
    ...
    def _run_phase2_batched(
        ...
        """Q7=B: flush_to_ltm is NOT called. STM is retained as the QA <CONTEXT>
        exactly as left at the end of Phase 1 (= last block's messages)."""
        phase2_stats = [
            {
        ...
        for i, (context, agent) in enumerate(zip(contexts, agents)):
            ...
            for qa_idx, qa in enumerate(context.qa_pairs):
                ...
                retrieved_metadata = _remap_retrieved_memories(relevant_memories)
        ...
        for start in tqdm(range(0, len(qa_jobs), cfg.QA_BATCH_SIZE), desc="Phase2 QA chunks"):
            ...
            for entry, result, usage in zip(chunk, results, usages):
                ...
                _add_tokens(
                    phase2_stats[entry["ctx_idx"]]["call_5_qa"],
                    usage["prompt_tokens"], usage["completion_tokens"],
                )

                raw_answer = result.get("answer", "") if isinstance(result, dict) else ""
                answer = agent.generator.normalize_letter(raw_answer)

                qa_results_per_context[entry["ctx_idx"]].append({
                    "question":                    entry["qa"].question,
                    "question_type":               entry["qa"].question_type,
                    "topic":                       entry["qa"].topic,
                    "all_options":                 entry["qa"].all_options,
                    "generated_answer":            answer,
                    "ground_truth_answer":         entry["qa"].correct_answer,
                    "end_index_in_shared_context": entry["qa"].end_index_in_shared_context,
    ...
    def run_batch(self, contexts: List[PersonaMemContext]) -> List[Dict[str, Any]]:
        agents = []
        log_writers = []

        for context in contexts:
            sample_id = (
                f"ldagent_{cfg.extract_model_name(self.model_path)}_"
            ...
            if cfg.ENABLE_LLM_CALL_LOGGING:
                llm_log_dir = self.prompt_log_dir / f"session_{context.context_index}"
                agent.set_llm_logger(LLMCallLogger(llm_log_dir))
            agents.append(agent)
            log_writers.append(
                RetrievalLogWriter(
                    self.retrieval_log_dir / f"session_{context.context_index}_retrieval_log.jsonl"
                )
            )
        ...
        for i, (context, agent) in enumerate(zip(contexts, agents)):
            ...
            if cfg.SAVE_MEMORY_SNAPSHOTS:
                snap_dir = self.memory_snapshots_dir / f"session_{context.context_index}"
                agent.save_snapshot(snap_dir)
                memory_snapshot_path = f"memory_snapshots/session_{context.context_index}"
            ...
            results.append({
                ...
                "token_statistics": {
                    ...
                    "call_4_summarization": p1["call_4_summarization"],
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run LD-Agent Batch Experiment on PersonaMem (QA-Only)")
    parser.add_argument("--start-session",   type=int, required=True)
    parser.add_argument("--end-session",     type=int, required=True)
    parser.add_argument("--benchmark-size",  type=str, required=True, choices=cfg.BENCHMARK_SIZES)
    ...
    cfg.ensure_directories(args.model, args.benchmark_size, args.start_session, args.end_session, config_name)
    ...
    logger.info("=" * 60)
    logger.info("LD-Agent Batch Experiment — PersonaMem (QA-Only)")
    ...
    if args.end_session >= len(all_contexts):
        logger.error(
            f"end_session={args.end_session} out of range (dataset has {len(all_contexts)} contexts)"
        )
        return 1
    ...
    print(f"\n{'#' * 60}")
    print(f"# LD-Agent Batch  |  benchmark_size={args.benchmark_size}")
    ...
    for batch_idx in range(num_batches):
        ...
        for result in batch_results:
            ...
            logger.info(f"Context {result['context_index']} complete.")

    logger.info("All contexts completed.")
    print_completion_banner(results_file, len(target_contexts))
    return 0
