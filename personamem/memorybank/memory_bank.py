"""
MemoryBank Memory System — PersonaMem variant

Core module implementing the MemoryBank memory mechanism. Internal variable
names (`conv_id`, `session_id`, `turn_id`, `CONVS_PER_DAY`) are retained from
the ImplexConv variant; the runner injects `block_idx` into the `conv_id` slot,
`context_index` into the `session_id` slot, and `local_msg_idx` into the
`turn_id` slot. Result metadata keys are remapped in run_experiment.py.

- Embedding store for dialogue messages (no LLM calls at storage time)
- Ebbinghaus forgetting curve: probabilistic permanent deletion at block boundaries
...
class MemoryBankSystem:
    ...
    def build_daily_summary_prompts(
        ...
        budget = int(cfg.FINALIZE_INPUT_CONTEXT_LIMIT * cfg.FINALIZE_CONTEXT_UTILIZATION) - self.summarize_max_tokens
        dialogue_text = _truncate_oldest_first(dialogue_text, budget)
        return (
    ...
    def build_global_summary_prompts(self) -> Optional[Tuple[str, str]]:
        ...
        sorted_event       = sorted(self.daily_event_summaries.items())
        sorted_personality = sorted(self.daily_personality_summaries.items())

        budget = int(cfg.FINALIZE_INPUT_CONTEXT_LIMIT * cfg.FINALIZE_CONTEXT_UTILIZATION) - self.summarize_max_tokens
        original = len(sorted_event)
        while len(sorted_event) > 1:
            event_tokens       = _count_tokens("\n".join(f"Day {c}: {s}" for c, s in sorted_event))
            personality_tokens = _count_tokens("\n".join(f"Day {c}: {s}" for c, s in sorted_personality))
            if event_tokens + personality_tokens <= budget:
                break
            sorted_event.pop(0)
            if sorted_personality:
                sorted_personality.pop(0)
        if len(sorted_event) < original:
            logger.warning(
                "global summary truncated from %d to %d days to fit %d-token budget",
                original, len(sorted_event), budget,
            )

        daily_event_text = "\n".join(f"Day {cid}: {summary}" for cid, summary in sorted_event)
        daily_personality_text = "\n".join(f"Day {cid}: {summary}" for cid, summary in sorted_personality)
