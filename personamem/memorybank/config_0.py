"""
Configuration for MemoryBank Batch Experiment on PersonaMem
"""
...
# Number of QA retrieval hits to keep when constructing the QA prompt.
RETRIEVE_K = 6

# Ebbinghaus forgetting curve: retention = exp(-(conv_gap / CONVS_PER_DAY) / (FORGETTING_DIVISOR * S))
FORGETTING_DIVISOR = 5

# How many block_idxs equal one "day" in PersonaMem.
# 1 block = 1 virtual day → daily summary at every block boundary.
CONVS_PER_DAY = 1

# Run global summary synthesis every N block_idxs.
# Phase 1 end always triggers a final global synthesis unconditionally.
GLOBAL_SUMMARY_INTERVAL = 10

# Minutes per local message (declared for cross-module consistency).
MINUTES_PER_TURN = 10

# Number of recent blocks to include in QA prompt history.
HISTORY_CONV_WINDOW = 1

# Summarization LLM settings (separate from QA generation).
SUMMARIZE_TEMPERATURE = 0.7
SUMMARIZE_MAX_TOKENS  = 400

# Input-side token budget for finalize LLM calls.
...
# Number of sub-blocks to split each block into for summarization granularity.
