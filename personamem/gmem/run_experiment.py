"""
GraphMem v6 Experiment Runner — PersonaMem (Batched, QA-Only)
...
def setup_logging(log_dir: Path) -> logging.Logger:
    ...
    if cfg.LOG_TO_FILE:
        handlers.append(logging.FileHandler(
            log_dir / f"gmem6_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="GraphMem v6 Experiment on PersonaMem")
    ...
    logger.info("=" * 60)
    logger.info("GraphMem v6 Experiment — PersonaMem")
