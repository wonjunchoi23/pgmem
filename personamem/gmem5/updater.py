...
class GraphUpdater:
    ...
    def _build_5a_call(
        ...
        # Canonical pairs for IRRELEVANT-fallback (Change 16):
    ...
    def _build_5c_call(
        ...
        for pair_num, (id_a, id_b) in enumerate(raw_pairs):
            ...
            canonical_pairs.append((old_id, new_id))
    ...
    def _build_5d_call(
        ...
        for pair_num, (s_id, m_id) in enumerate(raw_pairs):
            ...
            canonical_pairs.append((m_id, s_id))
    ...
    def _build_state_new_rel_call(
        ...
        for prev_id in prev_ids:
            for new_id in new_ids:
                expected_pairs.append((prev_id, new_id))
    ...
    def _build_memory_new_rel_call(
        ...
        # Direction is fixed to source = new_memory; data block goes at the
        # bottom (option B). Trigger lives in apply_call_result.
        ...
        # Canonical pairs: new_memory→previous_memory first if present,
        # then new_memory→chunk_state in listed order.
        expected_pairs: List[Tuple[str, str]] = []
        if prev_ids:
            expected_pairs.append((new_ids[0], prev_ids[0]))
        for s_id in state_ids:
            expected_pairs.append((new_ids[0], s_id))
        return PendingLLMCall(
    ...
    def _store_evidence_edge(self, src_id: str, dst_id: str, relation: str) -> None:
        """Materialize an EVIDENCE edge.

        Per Change 16, IRRELEVANT judgments DO create edges so that future
        ⑤b/⑤c/⑤d candidate selection (gated by has_direct_edge) skips the
        pair instead of re-judging it.
        """
    ...
    def apply_irrelevant_fallback(self, call: PendingLLMCall) -> None:
        ...
        Called when JUDGMENT_RETRY is exhausted with missing/partial judgments so that
        future ⑤b/⑤c/⑤d candidate selection (gated by has_direct_edge) skips
        the pair instead of re-judging it (Change 16). Direction follows each
        call's canonical (already encoded in expected_pairs at build time)."""
        if not call.expected_pairs:
            return
        for src_id, dst_id in call.expected_pairs:
    ...
    def _format_state_list(self, node_ids: List[Optional[str]], anchor_conv_id: int, anchor_turn_id: int) -> str:
        ...
        for nid in ids:
            ...
            # Default form: drop (scope, current_decision_impact). The
            # scope-exposed form is rendered by _format_state_list_with_scope
            # for ②b / ⑤c only (state-state SHIFT_TO contexts).
    ...
    def _format_state_list_with_scope(
        ...
        """State-listing variant that exposes `scope` (but not impact) for
        ②b and ⑤c so the LLM can reason about whether a narrow topical
        preference is replacing a broader lifestyle constraint. See
        gmem5_prompt.md §3.2 for the scope-exposure exception."""
        ids = [nid for nid in node_ids if nid]
        if not ids:
            return "(none)"
        lines = []
    ...
    def _format_memory_list(self, node_ids: List[Optional[str]], anchor_conv_id: int, anchor_turn_id: int) -> str:
        ...
        for nid in ids:
            ...
            if node is None:
                continue
            elapsed = self._elapsed_str(node.conv_id, node.turn_id, anchor_conv_id, anchor_turn_id)
            # Change 10: drop (scope) annotation.
            lines.append(f"[{nid}] [{elapsed}]: {node.content}")
        return "\n".join(lines) if lines else "(none)"
...
def _resolve_id(id_map: List[str], idx) -> Optional[str]:
    """Resolve an integer index from LLM output to a node_id via id_map."""
