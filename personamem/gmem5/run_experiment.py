"""
GraphMem v5 Experiment Runner — PersonaMem (Batched, QA-Only)

Processes PersonaMem shared contexts in parallel by batching LLM calls
across contexts. Each context has its own graph memory state.

Key adaptation from ImplexConv:
- PersonaMem messages are individual (user / assistant) rather than paired.
  They are grouped by block_idx and paired up within each block before being
  passed to GraphMemModule, which expects (user_utterance, gt_response) pairs.
- "User: " / "Assistant: " prefixes embedded in content are stripped before
  passing to process_turn_core() to avoid double-prefixing.
- block_idx maps to conv_id (chunk boundary detection).
- pair_idx_in_block maps to turn_id.
- context_index maps to session_id internally.
- QA is multiple-choice (a/b/c/d); retrieved_memories keys are remapped to
  PersonaMem names in results.
...
def _get_turn_pairs(context: PersonaMemContext, chunk_factor: int = 1) -> List[Tuple]:
    ...
    that memory extraction fires chunk_factor times per block instead of once.
...
def setup_logging(log_dir: Path) -> logging.Logger:
    ...
    if cfg.LOG_TO_FILE:
        handlers.append(logging.FileHandler(
            log_dir / f"gmem5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ))
...
class GraphMemBatchRunner:
    ...
    def _run_phase1_batched(
        ...
        for pair_idx in tqdm(range(max_pairs), desc="Phase1 pairs"):
            ...
            # pre-turn calls (chunk-boundary memory/trait extraction)
            pending = []
        ...
        # finalize: last chunk memory/trait extraction
        pending = []
        for i, (context, module) in enumerate(zip(contexts, modules)):
...
def main():
    ...
    pre_parser.add_argument("--config", type=str, default="config_1")
    ...
    parser = argparse.ArgumentParser(description="GraphMem v5 Experiment on PersonaMem")
    ...
    logger.info("=" * 60)
    logger.info("GraphMem v5 Experiment — PersonaMem")
