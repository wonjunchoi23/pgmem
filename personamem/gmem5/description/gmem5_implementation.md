...
Uses the serialized retrieval object and does not mutate memory. PersonaMem uses a single multichoice prompt:
- `SYS_QA_MULTICHOICE` + `QA_PROMPT_MULTICHOICE`: four-option (a/b/c/d) selection over the retrieved memory. If the retrieved memory is insufficient or irrelevant, the model picks the option most consistent with the question on its own — i.e. there *is* a fallback to general reasoning when the memory does not pin the answer.

The prompt:
- describes `[Traits]` vs `[Challenged Traits]` semantics, including the `↳ shifted to:` and `↳ conflicting evidence:` sub-bullets that the retrieval serialization (§3.4) emits;
- describes `[Current Constraints]` as states to honor unless the current question explicitly overrides them;
- emits `reasoning` first, then `answer`, to elicit chain-of-thought before the final letter.
