...
| Parameter | Recommended | Description |
|-----------|-------------|-------------|
| `INCLUDE_RECENT_CONVERSATION_FOR_QA` | `False` | include recent conversation in QA serialization (PersonaMem default off) |
| `QA_CONTEXT_PAIRS` | 5 | number of most-recent (user, agent) pairs included in the QA prompt's `[Recent Conversation]` block when above flag is `True` |
...
| Parameter | Recommended | Description |
|-----------|-------------|-------------|
| `TIME_PER_CONV_ID_HOURS` | 24 (32k) / 12 (128k, 1M) | hours per `conv_id` increment; overridden at runtime from `CHUNK_FACTOR` |
| `TIME_PER_TURN_MINUTES` | 10 | minutes per `turn_id` increment |
| `CHUNK_FACTOR` | `{"32k": 1, "128k": 2, "1M": 2}` | sub-block chunking per benchmark size (PersonaMem-specific): each block is split into `CHUNK_FACTOR` virtual chunks so memory is extracted multiple times per block on longer benchmarks. `TIME_PER_CONV_ID_HOURS = 24 // CHUNK_FACTOR[size]` at runtime |
| `CONV_IDS_PER_DAY` | 1 | virtual days per `conv_id` (PersonaMem: 1 block = 1 virtual day) |
