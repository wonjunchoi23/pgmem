"""
Configuration for GraphMem v5 on PersonaMem.
"""
...
# Per-call output caps for the 9 internal calls (Change 15). Each is set
# conservatively (typically 3-5× the empirically expected output size).
...
# Memory extracts once per virtual chunk; trait every TRAIT_EXTRACTION_CHUNKS chunks.
# 32k: 1 chunk/block  → memory/block,    trait/2 blocks
# 128k/1M: 2 chunks/block → memory/½ block, trait/1 block
CHUNK_FACTOR = {"32k": 1, "128k": 2, "1M": 2}
...
def get_output_dir(model_path: str, benchmark_size: str, config_name: str = "config_1") -> Path:
