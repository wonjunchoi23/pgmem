"""
Configuration for OnlyLLM Upper-Bound Experiment on PersonaMem.

HISTORY_ALL_GIVEN=True: the QA prompt sees the full conversation history,
trimmed from the head if it exceeds the model's token budget.
"""
...
# Upper-bound: full conversation history shown to QA prompt (head-trimmed
# under the model's token budget). MAX_CONTEXT_MESSAGES is unused when
# HISTORY_ALL_GIVEN=True, kept here as a reference value.
