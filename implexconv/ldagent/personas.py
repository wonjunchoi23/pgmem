...
class Personas:
    ...
    @staticmethod
    def _normalize_trait(result) -> str:
        if not isinstance(result, dict):
            return ""
        return str(result.get("trait", "")).strip()

    def _build_trait_prompts(self, sentence: str) -> Tuple[str, str]:
        sys_prompt = (
    ...
    def build_user_trait_prompt(self, sentence: str) -> Tuple[str, str]:
        return self._build_trait_prompts(sentence)

    def build_agent_trait_prompt(self, sentence: str) -> Tuple[str, str]:
        return self._build_trait_prompts(sentence)

    def apply_user_trait_result(self, result) -> List[str]:
        self.last_user_token_info = self._extract_token_info(result)
        summarized_traits = self._normalize_trait(result)
    ...
    def apply_agent_trait_result(self, result) -> List[str]:
    ...
    def _user_traits_update(self, sentence: str) -> List[str]:
        ...
        sys_prompt, user_prompt = self.build_user_trait_prompt(sentence)
        ...
        try:
            ...
            self.apply_user_trait_result(result)
    ...
    def _agent_traits_update(self, sentence: str) -> List[str]:
        ...
        sys_prompt, user_prompt = self.build_agent_trait_prompt(sentence)
        ...
        try:
            ...
            self.apply_agent_trait_result(result)
