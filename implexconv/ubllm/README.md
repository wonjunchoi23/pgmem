# UBLLM — Batched QA-Only Baseline

Top-level `ubllm/` is the batched main implementation. The original sequential implementation and historical outputs are preserved under `ubllm/ubllm_sequential/`.
...
```bash
python merge_results.py ubllm config_lb_outputs_Llama-3.1-8B-Instruct_opposed
```
...
- `memory_at_qa_start`: always zeroed because `ubllm` has no standalone memory store
