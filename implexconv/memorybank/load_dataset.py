...
class Turn:
    ...
    def to_memory_content(self) -> str:
        """
        Utility helper returning the bare utterance text.

        The main batched runner currently stores memories via `to_message()`;
        this helper is retained as a lightweight formatting convenience.
        """
...
class Session:
    ...
    def get_turns_by_conv_id(self, conv_id: int) -> List[Turn]:
        """Convenience accessor used for inspection/debugging helpers."""
        return [t for t in self.turns if t.conv_id == conv_id]

    def get_unique_conv_ids(self) -> List[int]:
        """Return conv_ids in first-seen order for inspection/debugging helpers."""
        seen = set()
        result = []
...
if __name__ == "__main__":
    ...
    for subset in ("opposed", "supportive"):
        ...
        print(f"\nSession 0 — conv_ids: {s.get_unique_conv_ids()}")
