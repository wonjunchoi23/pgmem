...
class GraphRetriever:
    """Implements the PGMem retrieval pipeline."""
    ...
    def _graph_expansion(
        ...
        # Pull ±1 turn neighbors of seed_s/seed_m (S/M, same conv_id) into the
        # pool as plain members — recall safety net since state extraction is
        # sparse (≤1 per turn). These are NOT expansion drivers.
        turn_neighbor_ids: Set[str] = set()
        for seed_node in list(seed_s) + list(seed_m):
            for neighbor in self._graph.get_turn_neighbors(
                seed_node.node_id, types={NODE_S, NODE_E}, delta=1
            ):
