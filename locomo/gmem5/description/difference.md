...
prompts (most notably between `_NODE_TYPE_DESC` / `_EVID_DESC_FULL` and each
call's user prompt), and self-redundant sections appeared inside individual
user prompts (e.g., a `Final instruction:` block restating earlier task
constraints). Furthermore, semantically identical guidance across the four ⑤
calls and ②b was worded inconsistently. The goal of this change is to make
each prompt section express its content **exactly once** and to make shared
content **literally identical** wherever it appears.

**Original Behavior**
- `_NODE_TYPE_DESC` was duplicated verbatim in `updater.py` and `generator.py`
  and contained a `Quick test:` block restating the four-line definition.
- `_EVID_DESC_FULL` (system prompt for ②b, ⑤a, ⑤c) carried a `Calibration
  rules:` block whose items also appeared in each call's user prompt.
- `_LABEL_DISCIPLINE_BLOCK` contained a five-line `Self-check:` paragraph
  that restated the `domain_label` definition.
- ② state-extraction user prompt restated the State definition (already in
  `_NODE_TYPE_DESC`), restated `currently or recently valid persona signal`
  three times, and ended with a `Final instruction:` repeating the task.
- ②b user prompt re-listed the four-label enum (already in
  `_EVID_DESC_FULL`), re-defined `SHIFT_TO`, and contained a `Relation
  calibration` + `SHIFT_TO caution` + `Final instruction` triplet that
  restated calibration items and ordering already given upstream.
- ③ memory-extraction user prompt ended with a `Final instruction:`
  repeating the single-memory task. scope BROAD/NARROW were rendered across
  multiple lines.
- ④ trait-extraction user prompt restated the Trait definition (already in
  `_NODE_TYPE_DESC`) twice and ended with a `Final instruction:` repeating
  the task.
- ⑤a body and final instruction used `for each required pair` while
  ⑤b/⑤c/⑤d used `for each listed pair/candidate, in the listed order`.
- ⑤b similarity-mining preamble omitted `over unconnected ... pairs in the
  graph`; ⑤c and ⑤d wrapped their preamble across three lines while ⑤b had
  a one-line variant.
- ⑤c CONTRADICT calibration was missing `for` before `facts that can
  naturally coexist` and the trailing `with the state` qualifier.
- ②b had `is not SUPPORT` (lowercase) while ⑤a/⑤b/⑤c/⑤d had `is NOT
  SUPPORT` (uppercase).
- 2b vs 5c SHIFT_TO calibration items differed in wording (`unless the user
  explicitly says so` vs `unless one state explicitly invalidates the
  other`, `new state` vs `newer state`, etc.).

**New Behavior**
- `_NODE_TYPE_DESC` is defined **once** in `generator.py`. `updater.py`
  imports it (`from generator import _NODE_TYPE_DESC`). The `Quick test:`
  block is removed; the four-line definition is the single source of truth.
- `_EVID_DESC_FULL` carries **only** the four label definitions; the
  `Calibration rules:` block is removed. All calibration items live in the
  user prompts of the calls that need them, tailored to each call's pair
  family.
- `_LABEL_DISCIPLINE_BLOCK` is shortened: the `Self-check:` paragraph and
  the `casing-only / singular-plural / spacing-hyphenation` Hard-constraint
  line (already enforced by the apply-side `deduplicate_labels`) are
  removed. The `keywords:` and `domain_label:` definition lines are
  tightened. Net effect: ~22 lines → 13 lines.
- ② user prompt removes the State definition first paragraph, the
  `Self-check:` 3-question block, the `describe a currently or recently
  valid user-specific persona signal` line, the mid-prompt `Assign
  placeholder ids:` line, and the `Final instruction:` block. scope
  BROAD/NARROW are collapsed to a single line each.
- ②b user prompt is restructured: the duplicate label-enum lines and the
  `SHIFT_TO` redefinition are removed; the previous separate `For
  previous↔new:` / `For new↔new:` blocks are consolidated into a single
  `Direction rules:` block; new↔new-specific cautions move to a `new↔new
  pair caution:` block; all calibration rules are consolidated under a
  single `Calibration:` block; the `Final instruction:` block is removed.
- ③ user prompt removes the `Final instruction:` block. scope BROAD/NARROW
  collapse to single lines each.
- ④ user prompt removes (1) the first sentence of the Trait definition,
  (2) the `describe a generalized characteristic that persists across
  situations` line, (3) the `capture a stable preference, value,
  disposition, or habitual tendency` line, all of which restate
  `_NODE_TYPE_DESC`'s Trait definition; the `Final instruction:` block is
  removed.
- The four ⑤ calls share these phrasings literally:
  - **Similarity-mining preamble** (⑤b/⑤c/⑤d): single-line
    `The listed {candidates|pairs} are surfaced by global similarity mining
    over unconnected {state↔trait and memory↔trait | state-state |
    state-memory} pairs in the graph. Their semantic similarity does NOT,
    by itself, imply any relation. Judge each {candidate|pair}'s content
    directly.`
  - **`Topical similarity alone is NOT SUPPORT.`** (uppercase NOT) — also
    applied to ②b for consistency.
  - **CONTRADICT calibration meta line** — `Do not use CONTRADICT for
    unrelated topics or for facts that can naturally coexist with the
    {trait | state}.` (only varies the final noun).
  - **Output-count phrasing**: `Output exactly N judgments — one for each
    listed {pair|candidate}, in the listed order.` (in body and Final
    instruction; replaces ⑤a's `required pair` and `in the order specified
    above` variants).
  - **SHIFT_TO calibration items between ②b and ⑤c** (both state-state)
    use ⑤c's wording verbatim: `unless one state explicitly invalidates
    the other`, `the newer state only adds detail without invalidating the
    older state`, and `for a topic change without explicit replacement`.
- ⑤a/⑤b/⑤c/⑤d Final instruction blocks are kept (per-call output-count
  reinforcement is intentional, as is the index-key reminder for ⑤d's
  `state_id`/`memory_id` schema).
...
- `gmem5_prompt.md` §9 (⑤a), §10 (⑤b), §11 (⑤c), §12 (⑤d) — apply shared
  wording (preamble, casing, CONTRADICT calibration, output-count phrasing,
  SHIFT_TO calibration).
- `gmem5_implementation.md` §4.2 — drop the `Final instruction:` bullet
  from the ③ prompt-structure description.

**Implementation Notes**
- All apply-side parsers (`_validate_relation`,
  `_validate_relation_reduced`, schema enforcement) are unchanged; this is
  a prompt-only refactor.
...
- Repetition of important content is preserved where deliberate (e.g., the
  per-call output-count reinforcement and the schema-key reminder in each
  ⑤ call's Final instruction). Removed redundancies are limited to
  (a) duplicate enum nominations already enforced by `_EVID_DESC_*` +
  schema, (b) re-statements of label definitions already given in the same
  prompt, and (c) `Final instruction:` blocks whose content was a verbatim
  restatement of earlier `Task:` / `Judge:` / `Output requirements:` lines.
- Cross-call consistency (`is NOT SUPPORT`, similarity-mining preamble,
  CONTRADICT calibration, SHIFT_TO calibration) is checked by `grep`;
  future prompt edits that touch these strings should keep the four ⑤
  calls and ②b aligned.
...
**Motivation**
Two refinements after Change 13:
- (a) the two QA system prompts (`SYS_QA_OPPOSED`, `SYS_QA_SUPPORTIVE`) shared
  ~10 lines of identical text covering retrieval-block semantics and the
  Challenged Trait policy, but the lines were hard-coded twice;
- (b) the retrieval serialization always emitted every section header
  (including a `(none)` body for empty sections), and the `[Relevant States]`
  / `[Relevant Memories]` headers had no description while only `[Current
  Constraints]` did, making the section semantics asymmetric;
- (c) trait/state output mix skewed toward traits — measurement on
  `session_0_49` showed states dominating decision-relevant evidence but
  trait listings were noisier (~3% of traits were challenged anyway and
  ~60% of sessions had zero challenged traits).

**Original Behavior**
- `SYS_QA_OPPOSED` and `SYS_QA_SUPPORTIVE` each carried their own copy of the
  retrieval-block guidance: `[Traits]` description (1 line), `[Challenged
  Traits]` 3-line policy block, `Current Constraints` line, and the
  introductory `The information below describes...` line.
- `_serialize` (in `retriever.py`) emitted every section header even when
  empty, with a `(none)` body line.
- `[Relevant States]` and `[Relevant Memories]` headers had no header_note;
  only `[Current Constraints]` did.
- `K_T_FINAL = 6`, `K_SF = 8`, `K_STATE = 12`, `K_APS = 5`. The state pool
  `K_STATE` was smaller than `K_APS + K_SF = 13`, marginally insufficient.

**New Behavior**
- A new private constant `_QA_COMMON_RETRIEVAL_GUIDE` is defined in
  `generator.py`. Both `SYS_QA_OPPOSED` and `SYS_QA_SUPPORTIVE` consume it
  via string concatenation. Within the shared block, the four-line
  Challenged Trait policy is compressed to a single sentence:
...
- `_serialize`'s inner `add_lines(title, lines, header_note)` now **skips
  ...
  `(none)` line, no blank-line separator. The `[Recent Conversation]`
  omission for QA (when `INCLUDE_RECENT_CONVERSATION_FOR_QA = False`) is
  unchanged but now coheres with the general empty-section rule.
...
- Retrieval count parameters in `config_0.py` are rebalanced:
  - `K_T_FINAL`: `6 → 4` (fewer surfaced traits)
  - `K_SF`: `8 → 12` (more surfaced relevant states)
  - `K_STATE`: `12 → 17` (pool now ≥ `K_APS + K_SF = 17`, restoring
    sufficiency)
  - `K_APS` (5) and `K_TRAIT` (6) are unchanged.

**Affected Documents**
- `gmem5_prompt.md` §3.4 — empty-section omission, `[Relevant ...]`
  header notes, updated example.
- `gmem5_prompt.md` §13.1 / §13.2 — replace bodies; add description of
  `_QA_COMMON_RETRIEVAL_GUIDE`.
- `gmem5_config.md` §2 — update seed pool counts (`k_m`, `k_s`, `k_t`,
  `k_aps`) to match the new actual values.
- `gmem5_config.md` §4.1 — update `k_sf`, `k_m_final` to actuals; `k_t_final`
  was already documented as 4.
- `gmem5_config.md` §6 — update `k_aps` row to actual value (5).

**Implementation Notes**
- The QA common block keeps the exact `[Traits]` / `[Challenged Traits]`
  spelling so that the LLM's pattern-match against the serialization headers
  (`[Traits]`, `[Challenged Traits]`) is unaffected.
- Empty-section omission removes a small but consistent token tax across
  every QA call. The Challenged Trait guidance still applies whenever a
  `[Challenged Traits]` section is emitted; it is harmless when absent.
- Trait↓ State↑ rebalance assumes `K_STATE ≥ K_APS + K_SF`. If `K_SF` is
  raised further, raise `K_STATE` accordingly.
...
- (a) every internal call (②, ②b, ③, ③b, ④, ⑤a, ⑤b, ⑤c, ⑤d) shared a single
  output cap `MAX_TOKENS_INTERNAL = 5000`, which is far above what most calls
  ever produce. Over-provisioning a shared cap masks per-call divergence and
  inflates worst-case generation cost.
- (b) per-call prompt/output logging was guarded by a hardcoded
  `session_id in (0, 1, 2)` check inside `run_experiment.py`, with no config
  hook. Anyone wanting to widen or narrow the logged-session set had to edit
  the runner.

**Original Behavior**
- All 9 internal calls used `cfg.MAX_TOKENS_INTERNAL` (= 5000).
- LLM call logging gated by `if cfg.ENABLE_LLM_CALL_LOGGING and session.session_id in (0, 1, 2)`.

**New Behavior**
- `MAX_TOKENS_INTERNAL` is removed. Each of the 9 internal calls now uses its
  own per-call cap, set conservatively (typically 3-5× the empirically
...
- A new config parameter `LLM_CALL_LOG_FIRST_N_SESSIONS` (default `10`)
  ...
  now active iff `cfg.ENABLE_LLM_CALL_LOGGING` is `True` AND
  `session.session_id < cfg.LLM_CALL_LOG_FIRST_N_SESSIONS`.

**Affected Documents**
- `gmem5_config.md` §13 — replace the single `MAX_TOKENS_INTERNAL` row with
  the 9 per-call rows; add `ENABLE_LLM_CALL_LOGGING` and
  `LLM_CALL_LOG_FIRST_N_SESSIONS` rows.
- `gmem5_prompt.md` "Logging" footer — note the new session filter.

**Implementation Notes**
- ⑤a typically produces the largest output (one judgment per recent
  state/memory across the two-chunk window plus the previous trait), which
  is why its cap is the highest at 2500.
- ⑤d's cap stays small (800) because both state and memory candidate
  reservoirs are very low (`STATE_MEMORY_EXTRA_REL_TOPK = 2`).
- The session filter is a strict `<` so `LLM_CALL_LOG_FIRST_N_SESSIONS = 0`
  disables logging entirely (equivalent to `ENABLE_LLM_CALL_LOGGING = False`
  but more granular). The default `= 10` covers the first 10 sessions;
  raise it for deeper auditing or set to `1`/`2` to log only the smoke-test
  set.

---

### Change 16: Scope NARROW Prior, IRRELEVANT-as-Edge, Trait in c-Seed Expansion, Batched JUDGMENT_RETRY, Defensive ID Normalization

**Motivation**
A bundle of corrections driven by post-run analysis of `session_0_49`
prompt logs and stored-edge statistics:
- (a) `scope` was nearly 100% `BROAD` because the calls had no
  `If uncertain → NARROW` prior, so LLMs defaulted to the more general
  label.
...
- (d) the batched runner (`run_experiment.py:_drain_pending_calls`)
  ...
  (`updater.py:_run_call`) honored it.
- (e) defensive normalization for LLM ID outputs was missing: even
  though the schema enforces `integer` types, retry/repair paths could
  in principle yield strings.

**Original Behavior**
- ②/③/④ scope blocks contained no `If uncertain → NARROW` prior. Only
  `current_decision_impact` had its analogous `If uncertain → LOW` line.
- `_store_evidence_edge(..., IRRELEVANT)` was a no-op:
  `if relation == EVID_IRRELEVANT: return`.
...
- `_drain_pending_calls` ran one batched generation per call_type group
  with no empty-judgments retry. Empty `judgments` arrays passed straight
  through to apply, producing no edges.
- The "JUDGMENT_RETRY exhaustion fallback" was documented as
  "every expected pair treated as IRRELEVANT (no edges created)" — but in
  practice no fallback action ran, since the IRRELEVANT-as-edge change
  hadn't landed and there was no retry on the batched path anyway.
- `_resolve_id` strictly required `isinstance(idx, int)`; any string or
  bracketed form was dropped silently.

**New Behavior**
- A single line `If uncertain between BROAD and NARROW, choose NARROW.`
  is appended after the `scope:` definition block in ②, ③, and ④ user
  prompts. Mirrors the existing `current_decision_impact` `If uncertain
  → LOW` pattern.
- `_store_evidence_edge` no longer drops `IRRELEVANT`. All
  `_apply_*_result` methods route through this helper, so every
  IRRELEVANT judgment now creates an edge with the call's canonical
  direction. Sign propagation and node score counters already exclude
  `IRRELEVANT`, so support/contradict accounting is unchanged.
- `retriever.py` Step 1 c-seed → SOURCE expansion now admits
  `{NODE_M, NODE_S, NODE_T}`. Steps 2a/2b/2c (`_expand_from`,
  `get_shift_forward_reachable`) already included `NODE_T` and are
  unchanged.
- `PendingLLMCall` gains `expected_pairs: List[Tuple[str, str]]`. Each
  relation-extraction builder (②b/③b/⑤a/⑤b/⑤c/⑤d) populates this
  list at construction time using the call's canonical direction:
...
- `GraphUpdater.apply_irrelevant_fallback(call)` (and its module-level
  ...
  defensive guards: skip if either endpoint is missing or if a direct
  edge already exists.
- `_drain_pending_calls` mirrors the sequential `JUDGMENT_RETRY` policy:
  - after each batched generation, identify jobs whose
    `expected_judgment_count > 0` AND `judgments == []`;
  - re-batch those jobs with an appended hint `"Previous attempt
    returned empty judgments; you MUST output exactly N judgments."`;
  - up to `cfg.JUDGMENT_RETRY` retries; retry token usage accumulates on
    top of the original;
  - on exhaustion, `module.apply_irrelevant_fallback(call)` is invoked
    so that subsequent ⑤b/⑤c/⑤d candidate selection (gated by
    `has_direct_edge`) skips these pairs.
- `_resolve_id` accepts `int` plus defensive string forms (`"5"`,
  `" 5 "`, `"[5]"`, `"[ 2 ]"`). Booleans and unparseable strings still
  return `None`.

**Affected Documents**
- `gmem5_prompt.md` §4 (②), §6 (③), §8 (④) — scope blocks updated.
- `gmem5_prompt.md` §1.10 — empty-judgment retry now describes both the
  batched and sequential paths and the IRRELEVANT-fallback edge creation.
- `gmem5_storage_extraction.md` §3.3 — note that IRRELEVANT judgments DO
  produce edges (since gmem5).
- `gmem5_storage_extraction.md` §5 (or wherever ②b post-call rules live)
  — "EVIDENCE edges for every judgment, including IRRELEVANT".
- `gmem5_retrieval.md` §3.2 — Step 1 expansion includes `t`.

**Implementation Notes**
- `apply_irrelevant_fallback` is intentionally conservative: it stores
  IRRELEVANT only when neither node is missing AND no direct edge
  already exists. This avoids overwriting a SUPPORT/CONTRADICT/SHIFT_TO
  edge produced by a partial retry.
- ID normalization is defensive only. Under the current
  `additionalProperties: False` schema with `type: integer`, the LLM
  cannot produce non-integer IDs through guided_json. The relaxation
  guards against retry/repair paths that may bypass guided_json.
- Storage volume increases — every previously-judged-IRRELEVANT pair
  now has an edge. In return, ⑤b/⑤c/⑤d LLM-call volume drops because
  candidate pools shrink to truly unjudged pairs. Net cost trade-off is
  expected to favor LLM cost over edge memory.
...
## New Edge Schema Fields Summary

| Field | Type | Source change | Stored when |
|-------|------|---------------|-------------|
| `rationale` | `str` | Change 4 | relation ∈ {SUPPORT, CONTRADICT, SHIFT_TO} |
| `evidence_quote` | `{a: str, b: str}` | Change 4 | relation ∈ {SUPPORT, CONTRADICT, SHIFT_TO} |
...
| QA system-prompt shared block | hardcoded twice (~10 lines, including 4-line Challenged Trait policy) | single `_QA_COMMON_RETRIEVAL_GUIDE` constant; Challenged Trait policy compressed to one sentence | Change 14 |
| Empty serialization sections | header emitted with `(none)` body line | section omitted entirely | Change 14 |
| `[Relevant States]` / `[Relevant Memories]` headers | no description note | one-line header_note disambiguating from `[Current Constraints]` / `[Traits]` | Change 14 |
| `K_T_FINAL` (final trait count) | 6 | 4 | Change 14 |
| `K_SF` (final relevant-state count) | 8 | 12 | Change 14 |
| `K_STATE` (state pool size) | 12 | 17 (matches `K_APS + K_SF`) | Change 14 |
| Internal-call output cap | single `MAX_TOKENS_INTERNAL = 5000` shared by ②/②b/③/③b/④/⑤a/⑤b/⑤c/⑤d | 9 per-call caps (800-2500), each ~3-5× expected output | Change 15 |
| LLM call logging session filter | hardcoded `session_id ∈ (0,1,2)` in runner | `cfg.LLM_CALL_LOG_FIRST_N_SESSIONS` (default 10), strict `<` | Change 15 |
...
| JUDGMENT_RETRY exhaustion fallback | "no edges created" (purely documentary) | `apply_irrelevant_fallback(call)` writes IRRELEVANT edges for `call.expected_pairs` | Change 16 |
| `_resolve_id` input | strictly `isinstance(idx, int)` | also accepts `"5"`, `" 5 "`, `"[5]"`, `"[ 2 ]"` (defensive only) | Change 16 |
