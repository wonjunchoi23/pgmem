"""
Updater for GraphMem v5 on LoComo.
"""
...
SYS_MEMORY_EXTRACT = (
    ...
    "Summarize the recent conversation between two named speakers into one episodic memory.\n"
    "Do not classify relationships in this call.\n"
    "Respond in strict JSON.\n\n"
    + _NODE_TYPE_DESC
)
...
class GraphUpdater:
    ...
    def apply_call_result(self, call: PendingLLMCall, result: Dict) -> Optional[PendingLLMCall]:
        ...
        if call.call_type == "memory":
            ...
            if (
                ...
                prev_id_for_call = old_prev_memory_id if old_prev_memory_id else None
    ...
    def _build_memory_call(
        ...
        prompt = (
            "Create exactly one episodic memory node summarizing what happened in the recent conversation between the two speakers.\n"
            "The memory must:\n"
            "- be 1–2 sentences,\n"
    ...
    def _build_5a_call(
        ...
        recent_memories_block, m_ids = self._format_memory_list_indexed(
            memory_ids, anchor_timestamp_seconds, start_idx=len(trait_ids) + len(s_ids),
        )
        ...
        prompt = (
            ...
            "- For (memory, new_trait): use SUPPORT only when the memory captures concrete speaker behavior that the trait would predict; the memory must add evidential force beyond mere topic overlap.\n"
    ...
    def _build_5b_call(
        ...
        cand_mem_block, cm_ids = self._format_memory_list_indexed(
            [n.node_id for n in candidate_memories], anchor_timestamp_seconds,
    ...
    def _build_5d_call(
        ...
        for pair_num, (s_id, m_id) in enumerate(raw_pairs):
            ...
            elapsed_m = self._elapsed_str(m_node.timestamp_seconds, anchor_timestamp_seconds)
            same_session = (s_node.session_id == m_node.session_id)
            if same_session:
                relation_context = "same session."
            else:
                gap_str = format_session_gap(s_node.timestamp_seconds, m_node.timestamp_seconds)
        ...
        prompt = (
            ...
            "Each pair carries a \"Relation context\" line indicating whether the state and memory come from the same session.\n"
            "- \"same session\" pairs deserve extra scrutiny: same-session co-extraction alone is NOT SUPPORT. Look for an episodic fact in the memory that would still ground the state outside that session.\n"
            "- \"different sessions\" pairs come from temporally distinct episodes; SUPPORT here is most justified when the memory's events directly evidence the state.\n"
            "\n"
            f"Output exactly {num_pairs} judgments — one for each listed pair, in the listed order.\n"
            "Do not skip or duplicate pairs.\n"
            "\n"
    ...
    def _build_memory_new_rel_call(
        ...
        new_block, new_ids = self._format_memory_list_indexed(
            [new_memory_id], anchor_timestamp_seconds, start_idx=0,
        )
