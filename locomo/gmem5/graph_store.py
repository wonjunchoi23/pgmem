"""
Graph storage for GraphMem v5 on LoComo.
"""
...
class Node:
    ...
    current_decision_impact: Optional[str] = None
    retrieval_count: int = 1
    timestamp_seconds: float = 0.0   # real elapsed-time anchor (derived from LoComo session date)
    dia_id: str = ""                  # LoComo dialogue ID, e.g. "D1:3"
    speaker: str = ""                 # speaker this node represents ("" for session-level memory)
...
class HeterogeneousGraph:
    """In-memory graph for one GraphMem v5 sample."""
    ...
    def add_node(self, node: Node) -> None:
        # Enforce keywords/domain_label disjointness post-hoc (Change 2).
    ...
    def _ordinary_evidence_neighbors(self, node_id: str) -> List[Tuple[str, str]]:
        """Return (neighbor_id, subtype) pairs reachable via ordinary evidence edges (SHIFT_TO excluded)."""
        ...
        for dst_id, subtype in self._evid_out.get(node_id, {}).items():
            if subtype not in EVID_ORDINARY:
                continue
            dst_node = self._nodes.get(dst_id)
            if dst_node is None or dst_node.node_type not in valid_out_types:
                continue
            if dst_id in seen:
                continue
            seen.add(dst_id)
            result.append((dst_id, subtype))

        for src_id, subtype in self._evid_in.get(node_id, {}).items():
            if subtype not in EVID_ORDINARY:
                continue
            src_node = self._nodes.get(src_id)
            if src_node is None or src_node.node_type != node.node_type:
                continue
            if src_id in seen:
                continue
            seen.add(src_id)
            result.append((src_id, subtype))

        return result

    def get_ordinary_signed_reachable(self, start_id: str, hop_cap: int) -> Dict[str, str]:
        """Multi-hop sign propagation over SUPPORT / CONTRADICT / IRRELEVANT only.

        Sign table:
          - SUP × SUP → SUP, continue
          - SUP × CON → CON, stop
          - CON terminates immediately at any hop (no further expansion from CON nodes).

        Multi-path resolution: shortest path wins (level-based BFS); within the same
        level, SUP edges are processed before CON edges so SUP wins on ties.

        SHIFT_TO is not included.
        ...
        for hop in range(hop_cap):
            ...
            for node_id, cum_sign in frontier:
                if cum_sign != EVID_SUPPORT:
                    # CON nodes do not expand further.
                    continue
                for neighbor_id, edge_sub in self._ordinary_evidence_neighbors(node_id):
                    ...
                    if edge_sub == EVID_SUPPORT:
                        ...
                        if neighbor_id not in sup_candidates:
            ...
            for nid, sign in con_candidates.items():
                if nid in result:
                    continue
                result[nid] = sign
                # CON nodes are recorded but do not expand further.
    ...
    def save_snapshot(self, directory: Path) -> None:
        ...
        for nid in self._creation_order:
            ...
            nodes_data[nid] = {
                ...
                "scope": node.scope,
                "current_decision_impact": node.current_decision_impact,
    ...
    def load_snapshot(self, directory: Path) -> None:
        ...
        for idx, nid in enumerate(creation_order):
            ...
            self._nodes[nid] = Node(
                ...
                turn_id=d["turn_id"],
                scope=d.get("scope"),
                current_decision_impact=d.get("current_decision_impact"),
    ...
    def node_count_by_type(self) -> Dict[str, int]:
        counts = {NODE_C: 0, NODE_M: 0, NODE_S: 0, NODE_T: 0}
...
def format_elapsed_str(
    entry_timestamp_seconds: float,
    current_timestamp_seconds: float,
) -> str:
    """Render an entry's date as an absolute date string (e.g. '8 May 2023').
    Returns 'unknown date' when the entry timestamp is missing.
    """
    if entry_timestamp_seconds <= 0.0:
        return "unknown date"
    dt = datetime.fromtimestamp(entry_timestamp_seconds)
    day = dt.day
    return dt.strftime(f"{day} %B %Y")


def format_session_gap(
    timestamp_a_seconds: float,
    timestamp_b_seconds: float,
) -> str:
    """Compact-duration string for the absolute gap between two timestamps.
    ...
    """
    if timestamp_a_seconds <= 0.0 or timestamp_b_seconds <= 0.0:
        return "unknown gap"
    delta_minutes = abs(timestamp_a_seconds - timestamp_b_seconds) / 60.0
    return _format_compact_duration(delta_minutes)
...
def deduplicate_labels(
    ...
    """Enforce keywords/domain_label disjointness post-hoc.

    Any token in domain_label that case-insensitively matches a keyword is
    removed. Keywords win on overlap. Original case is preserved in keywords;
    domain_label is filtered.
    """
