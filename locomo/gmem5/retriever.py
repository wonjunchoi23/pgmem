...
class GraphRetriever:
    """Implements the GraphMem v5 retrieval pipeline (LoComo)."""
    ...
    def _cross_type_sup_con(
        ...
        SHIFT_TO contributes bidirectionally at support-ratio time:
        ...
        Memories have no SHIFT_TO edges.
        """
        sup = 0
        con = 0
    ...
    def _serialize(
        ...
        for node in aps:
            ...
            aps_lines.append(f"[{self._elapsed_str(node, current_timestamp_seconds)}] {node.content}")
        add_lines(
            "Current Constraints",
            aps_lines,
        ...
        for node in traits_stable:
            ...
            if key in seen_content:
                continue
            seen_content.add(key)
            trait_lines.append(f"[{self._elapsed_str(node, current_timestamp_seconds)}] {node.content}")
        ...
        for node in states_relevant:
            ...
            seen_content.add(key)
            state_lines.append(
                f"[{self._elapsed_str(node, current_timestamp_seconds)}] {node.content}"
            )
        add_lines(
            "Relevant States",
            state_lines,
        ...
        for node in memories_relevant:
            ...
            if key in seen_content:
                continue
            seen_content.add(key)
            memory_lines.append(f"[{self._elapsed_str(node, current_timestamp_seconds)}] {node.content}")
        add_lines(
            "Relevant Memories",
            memory_lines,
