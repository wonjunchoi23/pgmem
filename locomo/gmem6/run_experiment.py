...
class GraphMemBatchRunner:
    ...
    def run_batch(
        ...
        if qa_only:
            for sample, module in zip(samples, modules):
                snapshot_dir = snapshots_dir / f"sample_{sample.sample_id}"
                if not snapshot_dir.exists():
                    raise FileNotFoundError(
                        f"qa-only mode: snapshot not found for sample {sample.sample_id} at {snapshot_dir}"
                    )
                module.load_snapshot(str(snapshot_dir))
            phase1_token_stats = [
                {} for _ in samples
            ]
            phase1_internal_stats = [{} for _ in samples]
        else:
    ...
    def _run_phase1_batched(
        ...
        for turn_idx in tqdm(range(max_turns), desc="Phase1 turns"):
            ...
            # pre-turn calls (session boundary → episode extraction)
            pending = []
...
def main():
    ...
    cfg.ensure_directories(args.model, args.start_sample, args.end_sample, config_name, suffix)
    ...
    results_file = cfg.get_results_file(args.model, args.start_sample, args.end_sample, config_name, suffix)
    checkpoint_file = cfg.get_checkpoint_file(args.model, args.start_sample, args.end_sample, config_name, suffix)
    retrieval_log_dir = cfg.get_retrieval_log_dir(args.model, args.start_sample, args.end_sample, config_name, suffix)
    snapshots_dir = cfg.get_memory_snapshots_dir(args.model, args.start_sample, args.end_sample, config_name)
    prompt_log_dir = cfg.get_prompt_log_dir(args.model, args.start_sample, args.end_sample, config_name, suffix)
