...
parser = argparse.ArgumentParser(description="compress segments.")
parser.add_argument("--load_path", default="result/mtbp/gpt4seg_mtbp.jsonl")
parser.add_argument("--save_path", default="result/mtbp/llmlingua2comp_mtbp.jsonl")
...
for idx, sample in enumerate(tqdm(data)):
    if sample["conversation_id"] in processed_ids:
        ...
        continue
    print(f"compressing {idx}-th conversation")
    for granularity in ["segments", "sessions", "turns"]:
        units = sample[granularity]
        assert (
            isinstance(units, List)
            and isinstance(units[0], List)
            and isinstance(units[0][0], str)
        )
        sample[f"comp_{granularity}"] = secom.compress(units)
    results.append(sample)
