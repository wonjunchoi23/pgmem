"""
Configuration for MemoryBank Batch Experiment on LoComo
"""
...
TEMPERATURE = 0.7       # default temperature (category 1-4 QA)
TEMPERATURE_C5 = 0.5    # category 5 adversarial QA temperature
MAX_TOKENS = 750
JSON_RETRY = 3
...
# Number of memories to retrieve per query (after forgetting filter)
# Aligned with original MemoryBank paper (VECTOR_SEARCH_TOP_K = 6).
RETRIEVE_K = 6

# Ebbinghaus forgetting curve: retention = exp(-day_gap / (FORGETTING_DIVISOR * S))
# Original paper uses divisor = 5.
FORGETTING_DIVISOR = 5

# Summarization LLM settings (separate from QA generation)
SUMMARIZE_TEMPERATURE = 0.7
SUMMARIZE_MAX_TOKENS = 400

# Number of most-recent sessions whose turns are included in QA prompt history
HISTORY_SESSION_WINDOW = 1
...
# Number of samples to process in parallel per batch.
# Controls how many agents are active simultaneously (memory footprint).
BATCH_SIZE = 4

# Number of QA prompts per single batch LLM call.
# Can be much larger than BATCH_SIZE since QA prompts are independent.
QA_BATCH_SIZE = 64
