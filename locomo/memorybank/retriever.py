...
Batch variant: __init__ accepts either a model name string or an already-loaded
SentenceTransformer instance, enabling a single shared model across agents.
"""
...
class EmbeddingRetriever:
    ...
    def __init__(self, model_name_or_instance="all-MiniLM-L6-v2"):
        """
        Args:
            model_name_or_instance: Either a model name string (will be loaded)
                or an already-loaded SentenceTransformer instance (shared across agents).
        """
        if not ST_AVAILABLE:
            raise ImportError(
