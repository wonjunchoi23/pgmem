...
class HeterogeneousGraph:
    ...
    def get_turn_neighbors(
        ...
        """Nodes of `types` within ≤ `delta` turns of `node_id` in the same session.
        Excludes the anchor itself; never crosses session boundaries.
        """
        anchor = self._nodes.get(node_id)
        if anchor is None or delta <= 0:
            return []
        session_id = anchor.session_id
        turn_idx = anchor.turn_id
        result: List[Node] = []
        for nid in self._creation_order:
            ...
            if n.session_id != session_id:
                continue
            if abs(n.turn_id - turn_idx) > delta or n.turn_id == turn_idx:
                continue
            result.append(n)
        return result
    ...
    def get_ordinary_signed_reachable(self, start_id: str, hop_cap: int) -> Dict[str, str]:
        ...
        SHIFT_TO is folded into the sign mapping by `_ordinary_evidence_neighbors`.
