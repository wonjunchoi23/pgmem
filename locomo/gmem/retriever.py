"""
Retriever for GraphMem v5 on LoComo.
"""
...
class GraphRetriever:
    """Implements the GraphMem v6 retrieval pipeline (LoComo)."""
    ...
    def _seed_retrieval(
        ...
        # State seed retrieval + APS construction are partitioned:
        ...
        # rank K_APS naturally fall through into the s_seed pool.
    ...
    def _graph_expansion(
        ...
        # (S/E, same session) into the pool as plain members — recall safety
        ...
        # Step 2: sign-propagation expansion. Origins exclude APS members
        # (query-independent persona slot) and turn-neighbors (recall safety
        # net only); both still contribute SUP/CON signals to other nodes
        # through _cross_type_sup_con. SHIFT_TO is folded into sign
        # propagation (graph_store._ordinary_evidence_neighbors), so no
        # separate forward-only SHIFT_TO discovery pass is needed.
        aps_ids = {n.node_id for n in aps}
        origin_ids = [
    ...
    def _final_set_assembly(
        ...
        # ---- Trait selection ----
        scored_traits: List[Tuple[float, int, Node]] = []
    ...
    def _serialize(
        ...
        def add_dedup_lines(title: str, nodes: List[Node], header_note: str = "") -> None:
            lines: List[str] = []
            for node in nodes:
                ...
                lines.append(f"[{self._elapsed_str(node, current_timestamp_seconds)}] {node.content}")
            add_lines(title, lines, header_note)

        add_dedup_lines(
            "Current Constraints",
            aps,
            "(These are high-impact speaker states the assistant should honor in the response, unless the question explicitly overrides them.)",
        )

        add_dedup_lines("Traits", traits_stable)
        ...
        for trait in traits_challenged:
            ...
            challenged_lines.append(
                f"[{self._elapsed_str(trait, current_timestamp_seconds)}] {trait.content}"
            )
            ...
            for newer in conflicts.get("shifted_to", []):
                ...
                challenged_lines.append(
                    f"  ↳ shifted to: [{self._elapsed_str(newer, current_timestamp_seconds)}] {newer.content}"
                )
            for node, _relation in conflicts.get("states", []) + conflicts.get("episodes", []):
                ...
                local_seen.add(snippet)
                challenged_lines.append(
                    f"  ↳ conflicting evidence: [{self._elapsed_str(node, current_timestamp_seconds)}] {node.content}"
                )
        ...
        add_dedup_lines(
            "Relevant States",
            states_relevant,
            "(Additional retrieved speaker states pertaining to the current question; not necessarily high-impact.)",
        )
    ...
    def _elapsed_str(self, node: Node, current_timestamp_seconds: float) -> str:
        return format_elapsed_str(node.timestamp_seconds, current_timestamp_seconds)
