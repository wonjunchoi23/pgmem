"""
Configuration for Theanine Experiment on PrefEval (implicit-persona).

Adapted from exp_implexconv_no_response/theanine/config_0.py.
Differences vs ImplexConv config:
- Single dataset path (no opposed/supportive split).
- MAX_TOKENS kept at 2048 (theanine multi-stage reasoning needs headroom);
  QA word cap is enforced via prompt template ("maximum 200 words").
...
- FINALIZE_EVERY_N_CONVS kept at 2 (= CONV_IDS_PER_DAY) so natural
  finalize-at-2-conv batching from the original ImplexConv adapter is
  preserved. Run-time runner does NOT force-flush at checkpoints.
"""
