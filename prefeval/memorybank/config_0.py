"""
Configuration for MemoryBank Experiment on PrefEval (implicit-persona).

Adapted from exp_implexconv_no_response/memorybank/config_0.py.
...
# In PrefEval chain mode, daily summary + global synthesis are forced at every
# checkpoint to ensure user_portrait reflects all sessions ingested so far.
# This matches the original MemoryBank behaviour (1 date = 1 daily summary,
# overall always re-synthesized after dailies). The natural day-boundary
# logic in run_experiment.py is therefore effectively a no-op (day_batch is
# emptied at every checkpoint), but we keep apply_forgetting on conv boundaries.
