"""
A-MEM Agent Module — PrefEval port.

Trimmed from exp_implexconv_no_response/amem/agent.py:
- Removed `subset` (no opposed/supportive split).
- Removed QA_PROMPT_SUPPORTIVE / QA_SCHEMA_SUPPORTIVE.
- QA word cap: 100 → 200 words.
- MAX_TOKENS upstream raised to 1500 (in config_0.py).

Otherwise unchanged: same memory_layer interface, token tracking, snapshot
helpers, prompt-too-long retry logic.
"""
...
class BaseAgent:
    """A-MEM agent wrapping AgenticMemorySystem (single-chain variant)."""
