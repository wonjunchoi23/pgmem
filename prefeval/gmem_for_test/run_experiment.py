"""
GraphMem v6 Experiment Runner — PrefEval (single-chain, cumulative-checkpoint).
...
3. Init shared embedding model + spaCy + LLM client; create GraphMemModule.
   If k_existing >= 0: load_snapshot(m_{k_existing}); start_k = k_existing+1.
   Else:               start_k = 0.
4. Load PrefEval samples [0..K] as a chain of Sessions.
5. For k = start_k .. K:
     a. Ingest session k turn-by-turn (pre/post drain per turn, finalize at end).
...
def setup_logging(log_dir: Path) -> logging.Logger:
    ...
    if cfg.LOG_TO_FILE:
        log_file = log_dir / f"gmem_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class ChainRunner:
    """Single-module chain runner with gmem's per-turn batched-drain ingestion."""
    ...
    # ------------------------------------------------------------------
    # Batched-drain (carried over from PersonaMem gmem; jobs list length = 1)
    # ------------------------------------------------------------------
...
def main():
    ...
    # ---- Step 2: parse args ----
    parser = argparse.ArgumentParser(description="GraphMem v6 PrefEval Chain Experiment")
    ...
    parser.add_argument("--config", type=str, default="config_8")
    ...
    logger.info("=" * 60)
    logger.info("GraphMem v6 PrefEval Chain")
    ...
    print(f"\n{'#'*60}")
    print(f"# GraphMem PrefEval Chain  |  model={cfg.extract_model_name(args.model)}")
