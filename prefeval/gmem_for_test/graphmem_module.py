"""Top-level GraphMem module — PrefEval variant (free-form QA only)."""
...
class GraphMemModule:
    """Top-level GraphMem orchestrator."""
    ...
    def prepare_qa(self, question: str) -> PreparedQA:
        context_cache_str = self._context_cache.get_formatted_context(
            current_conv_id=self._current_conv_id,
            current_turn_id=self._current_turn_id,
            cfg=self._cfg,
        ...
        prompt, system_prompt, schema = self._generator.build_qa_prompt(
            question=question,
            retrieved_episode=retrieval_result.serialized,
        )
        retrieved_memories = [
            {
    ...
    def get_qa_answer(self, question: str) -> QAResult:
        prepared = self.prepare_qa(question)
        answer, token_info, prompt_snapshot = self._generator.answer_qa(
            question=question,
            retrieved_episode=prepared.retrieval_result.serialized,
        )
        return QAResult(
            answer=answer,
