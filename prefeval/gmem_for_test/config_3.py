"""Configuration for GraphMem v6 on PrefEval (implicit-persona).

Adapted from exp_personamem/gmem/config_8.py.
Differences vs PersonaMem config:
- Single dataset path (no benchmark sizes, no opposed/supportive split).
- Output dir keyed by model only (no benchmark_size, no session range).
- MAX_TOKENS bumped from 1000 to 1500 (free-form 200-word answers).
- CONV_IDS_PER_DAY=2, TIME_PER_CONV_ID_HOURS=12 (PrefEval spec §3).
- CHUNK_FACTOR is a scalar (was per-benchmark-size dict).
- BATCH_SIZE / per-context batching removed (chain count is always 1).
- Multichoice QA prompt knobs removed.
"""
...
# Seed retrieval top-k (config_8 variant — slightly tighter than config_0)
K_CONTEXT = 24
K_EPISODE = 8
K_EPISODE_FINAL = 5
K_STATE = 20
