...
Changes from v3:
  - ChromaDB replaced with numpy + SentenceTransformer for LTM storage:
      ...
      * _encode(), _ltm_add(), _ltm_query() replace chromadb API calls
      * save_snapshot() writes ltm_embeddings.npy + long_term_memory.json
      * load_snapshot() reads ltm_embeddings.npy; falls back to re-encoding if absent
  - get_memory_stats() added for memory-at-QA-start logging
  - memory_cache_path parameter kept for interface compatibility but is unused
