...
class OpenAIClient(BaseLLMClient):
    ...
    reasoning_mode: for newer reasoning models (gpt-5*, o1*, o3*) that require
    `max_completion_tokens` instead of `max_tokens` and only allow temperature=1.
    """
        ...
        **kwargs
        ...
        if self.reasoning_mode:
            api_kwargs = {
                ...
                "temperature": 1,
                **{k: v for k, v in kwargs.items() if k not in ("max_tokens", "temperature", "return_usage", "json_retry")},
            }
        else:
