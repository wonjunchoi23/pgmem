...
three dedicated components:
  ...
  Generator         → QA answering

Phase 1 interface (per turn):
  retrieve_for_response(query)                     → (timelines, log_items)  [retrieval only, no LLM]
...
Token categories (for run_experiment.py aggregation):
  ...
  call_5_qa            — answer_qa generation LLM call
"""
...
class TheanineModule:
    ...
    Token tracking:
    - Memory tokens: accumulated in memory_graph, collected via get_and_reset_token_counts()
      (called by run_experiment.py after finalize_conv).
    - Refine tokens: accumulated in timeline_retriever, returned directly from
      generate_response() as refine_tokens.
    - Response / QA tokens: returned directly from generate_response() / answer_qa().
    """

    def __init__(self, llm_client, model_path: str = ""):
        self.llm_client  = llm_client
        self.model_path  = model_path
        self.memory_graph = MemoryGraph(llm_client, cfg)
    ...
    def retrieve_for_response(self, query: str) -> Tuple[Dict, List[Dict]]:
        """
        Retrieve timeline paths for response generation.
        ...
        to generate_response) and log_items for retrieval logging.

        If the memory graph is empty, returns empty timelines gracefully.

        Args:
            query: Current accumulated dialogue string (up to and including the
                   current user utterance) used as retrieval query.
    ...
    def finalize_conv(
        ...
        Returns:
            {"input": ..., "output": ..., "api_calls": ...}  — token counts.
        """
        logger.info(
    ...
    def answer_qa(
        ...
        Mirrors generate_response() exactly — uses the same full pipeline:
        ...
        retrieve_for_response() should be called first to obtain timelines;
        ...
        Args:
            ...
            timelines:        Dict from retrieve_for_response() (or retrieve_timeline()).
            subset:           "opposed" (free-form) or "supportive" (yes/no/unknown).
            current_dialogue: Full session dialogue with question appended as
                              the last User turn (same for every QA question).
        ...
        Returns:
            ...
            refine_tokens: {"input": ..., "output": ..., "api_calls": ...}
                           → counted as other_tokens in token_statistics.
            prompt_snapshot: Full prompt sent to Generator.
        """
        use_timelines = timelines.get("use_timeline", [])

        # Timeline refinement (other tokens) — identical to generate_response()
        refined_texts, refine_tokens = self.timeline.refine_all(
            use_timelines, current_dialogue, self.memory_graph
        )
    ...
    # =========================================================================
    # Memory stats & token tracking helpers
    # =========================================================================

    def get_memory_stats(self) -> Dict:
        """
        Return current memory node count and estimated total content tokens.
        Call after Phase 1 ends, before Phase 2 begins.
    ...
    def get_and_reset_memory_tokens(self) -> Dict:
        ...
        """
        return self.memory_graph.get_and_reset_token_counts()
