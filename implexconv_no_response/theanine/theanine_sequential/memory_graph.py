"""
Memory Graph for Theanine-New — ImplexConv
...
Adaptations for exp_implexconv:
  - Online processing: memory nodes created at conv_id boundaries
    (original: offline batch preprocessing per session)
  - Node key format: "c{conv_id}-m{idx}"  (original: "s{session_num}-m{idx}")
  - Dialogue context for relation extraction: full conv_id dialogue
    (original: full session dialogue)
  - sentence-transformers for local embeddings
    (original: OpenAI text-embedding-3-small)
  - llm_client.py (vLLM / Together / OpenAI) instead of LangChain + OpenAI API
  - Memory accumulates across conv_ids within a session; cleared between sessions

Mapping:
  original "session" (s1, s2, ...)  ≈  conv_id in this experiment
  original "episode" (all sessions)  ≈  session in this experiment
"""
...
class MemoryGraph:
    ...
    def _summarize_conv(self, dialogue: str) -> Tuple[List[str], Dict]:
        ...
        try:
            ...
            fallback_prompt = SUMMARIZATION_PROMPT_PLAINTEXT.format(dialogue=dialogue)
            try:
                ...
                text = (
                    fb_result.get("content", "") if isinstance(fb_result, dict)
                    else str(fb_result)
                )
    ...
    def finalize_conv(
        ...
        if not sentences:
            ...
            return {
                "call_3_summarization": {
                    "input": summ_tokens["input"],
                    "output": summ_tokens["output"],
                    "llm_calls": 1,
                    "parse_fallback_count": fallback_delta,
                },
        ...
        return {
            ...
            "call_4_relation": {
                "input":     total_relation_input,
                "output":    total_relation_output,
                "llm_calls": relation_calls,
            },
        }
    ...
    def get_and_reset_token_counts(self) -> Dict:
        ...
        extraction) and reset all counters to zero.
