"""
Configuration for Theanine Experiment on ImplexConv
...
Theanine-specific hyperparameters (from original Theanine paper):
  ...
  - RETRIEVE_TOP_K   = 3   (k: top-k nodes retrieved for response generation)
  - TIMELINE_SAMPLE_N = 1  (n: number of timeline paths sampled per retrieved node)
  - QA_RETRIEVE_K    = 3   (top-k nodes for QA phase, simple retrieval)
"""
...
TEMPERATURE          = 0.7
MAX_TOKENS           = 750   # response generation, relation extraction, refinement
...
# k: top-k memory nodes retrieved for response generation (timeline retrieval)
RETRIEVE_TOP_K = 3

# n: number of timeline paths sampled per retrieved node
TIMELINE_SAMPLE_N = 1

# top-k nodes retrieved for QA (Phase 2, simple retrieval — no timeline)
QA_RETRIEVE_K = 3
