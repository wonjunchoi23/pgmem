...
------
    ...
    sessions = load_implexconv_dataset("opposed")   # or "supportive"
...
class Turn:
    ...
    def to_message(self, elapsed_label: str = "") -> str:
        """Format as '[X days ago] User: ...' or 'User: ...' when no label."""
        role_label = "User" if self.role == "user" else "Assistant"
        prefix = f"{elapsed_label} " if elapsed_label else ""
        return f"{prefix}{role_label}: {self.utterance}"
...
def compute_virtual_minutes(
    ...
    """
    Compute the virtual time (in minutes from session start) for a turn.

    Time model:
        - conv_ids_per_day consecutive conv_ids = 1 virtual day
    ...
    Returns:
        Virtual time in minutes (float).
    """
    minutes_per_day = 24 * 60
    day = conv_id // conv_ids_per_day
    half = conv_id % conv_ids_per_day          # intra-day conv index (0 or 1)
    minutes_per_half = minutes_per_day / conv_ids_per_day
    return day * minutes_per_day + half * minutes_per_half + turn_id * minutes_per_turn


def format_elapsed_label(elapsed_minutes: float) -> str:
    """
    Format an elapsed time (in minutes) as a human-readable label.

    Examples:
        0   → "[just now]"
        45  → "[45 minutes ago]"
        90  → "[1 hour 30 minutes ago]"
        60  → "[1 hour ago]"
        4320 (3 days) → "[3 days ago]"
        4380 (3 days 1 hour) → "[3 days 1 hour ago]"

    Zero components are omitted (e.g. "1 hour ago" not "1 hour 0 minutes ago").
    """
    total = int(round(elapsed_minutes))
...
# =============================================================================
# CONTEXT UTILITIES  (used by run_experiment.py)
# =============================================================================

def get_context_window(turns: List[Turn], current_index: int, window_size: int = 30) -> List[Turn]:
    """Return up to `window_size` turns immediately before `current_index`."""
    start = max(0, current_index - window_size)
    return turns[start:current_index]


def format_context_for_prompt(turns: List[Turn]) -> str:
    """Format a list of turns into a multi-line context string."""
    if not turns:
